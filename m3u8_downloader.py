...
class M3U8Downloader:
    def __init__(self, url, output_dir="downloads", output_name=None, max_workers=10):
        """
        初始化M3U8下载器
    ...
    def download_m3u8(self):
        ...
        try:
            ...
            if playlist.is_variant:
                ...
                response = requests.get(variant_url, headers=self.headers, timeout=30)
                response.raise_for_status()
                playlist = m3u8.loads(response.text)
    ...
    def get_decrypt_key(self, key_obj):
        ...
        try:
            ...
            response = requests.get(key_url, headers=self.headers, timeout=30)
            response.raise_for_status()
